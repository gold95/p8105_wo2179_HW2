...
```
Convert 'entry' variable from character to logical: created a variable enter_df and modified it

```{r enter_df, warning = FALSE }
...
Modified NYC_df via mutate to change entry into entry_df which is now a logical variable
```{r}
